...
% Extract submodels:
longistates = [imap.x('Va_mps') imap.x('nz') imap.x('theta_rad') imap.x('q_rps') imap.x('x_NED_m') imap.x('z_NED_m')];
longiinputs = [imap.u('de_rad') imap.u('den_0to1') imap.u('vw_x_mps') imap.u('vw_z_mps') imap.u('omegaw_y_rps')];
longioutputs = [imap.y('Va_mps') imap.y('nz') imap.y('q_rps') imap.y('nz') imap.y('x_NED_m') imap.y('z_NED_m')];
...
longistates = [imap.x('Va_mps') imap.x('nz') imap.x('theta_rad') imap.x('q_rps')];
longiinputs = [imap.u('de_rad') imap.u('den_0to1')];
longioutputs = [imap.y('Va_mps') imap.y('nz') imap.y('theta_rad') imap.y('q_rps')];
linear.eulers.ss_longi_speed_withoutwind = getSubsystem(linear.eulers.ss_full, ...
    longistates, longiinputs, longioutputs);

lateralstates = [imap.x('beta_rad') imap.x('phi_rad') imap.x('psi_rad') imap.x('p_rps') imap.x('r_rps') imap.x('y_NED_m')];
lateralinputs = [imap.u('da_rad') imap.u('dr_rad') imap.u('vw_y_mps') imap.u('omegaw_x_rps') imap.u('omegaw_z_rps')];
lateraloutputs = [imap.y('beta_rad') imap.y('phi_rad') imap.y('psi_rad') imap.y('p_rps') imap.y('r_rps') imap.y('y_NED_m')];
linear.eulers.ss_lateral = getSubsystem(linear.eulers.ss_full, ...
    lateralstates, lateralinputs, lateraloutputs);

x = [imap.x('beta_rad') imap.x('phi_rad') imap.x('psi_rad') imap.x('p_rps') imap.x('r_rps') imap.x('y_NED_m')];
u = [imap.u('da_rad') imap.u('dr_rad')];
y = [imap.y('beta_rad') imap.y('phi_rad') imap.y('psi_rad') imap.y('p_rps') imap.y('r_rps') imap.y('y_NED_m')];
...
shortperiodstates = [imap.x('nz') imap.x('q_rps')];
shortperiodinputs = [imap.u('de_rad')];
shortperiodoutputs = [imap.y('nz') imap.y('q_rps')];
linear.eulers.ss_shortperiod = getSubsystem(linear.eulers.ss_full, ...
    shortperiodstates, shortperiodinputs, shortperiodoutputs);

ix = [imap.x('nz') imap.x('theta_rad') imap.x('q_rps')];
iu = [imap.u('de_rad')];
iy = [imap.y('nz') imap.y('theta_rad') imap.y('q_rps')];
...
try
    ...
    x = [imap.x('y_NED_m') imap.x('psi_rad') imap.x('phi_rad')];
    u = [];
    y = [imap.y('y_NED_m') imap.y('psi_rad')];
    sys = getSubsystem(linear.eulers_position.ss_full, x, u, y);
...
try
    imap = linear.eulers.imap;
    ix = [imap.x('nz') imap.x('q_rps')];
    iu = [imap.u('de_rad') imap.u('vw_z_mps') imap.u('omegaw_y_rps')];
    iy = [imap.y('nz') imap.y('nz') imap.y('q_rps')];
